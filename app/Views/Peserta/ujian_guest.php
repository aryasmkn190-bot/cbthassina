...
<script>
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            url: '<?= base_url('share/ujian/get-ujian/' . $ujianId) ?>',
            method: 'GET',
            dataType: 'json',
        ...
        if (statusUjian === 'mulai') {
            ...
            $.getJSON("<?= base_url('share/ujian/soal/') . $ujian['id'] ?>", function(res) {
                if (res.status) {
                    soalList = res.data;
    ...
    function afterSoalLoaded() {
        ...
        if (Object.keys(cache).length === 0) {
            ...
            $.get('<?= base_url('share/ujian/get-jawaban/' . $ujian['id']) ?>', function(res) {
    ...
    function simpanJawabanAktif(force = false) {
        ...
        if (force || jawabanString !== cache[soalId + '_last']) {
            ...
            $.ajax({
                url: '<?= base_url('share/ujian/simpan-jawaban') ?>',
                method: 'POST',
                data: dataPost,
                dataType: 'json',
...
<script>
    $(document).ready(function() {
        $('#ujianForm').on('submit', function(e) {
            ...
            $.ajax({
                url: '<?= base_url('share/ujian/selesai/' . $ujian['id']) ?>',
                method: 'POST',
                data: $(this).serialize(),
                dataType: 'json',
                success: function(res) {
                    if (res.status) {
                        ...
                        window.location.href = res.redirect || '<?= base_url('share/ujian/play/' . $ujian['token']) ?>';
                    } else {
                        Swal.fire({
                            icon: 'error',
                        ...
                        // kasih jeda 3 detik sebelum redirect
                        setTimeout(function() {
                            window.location.href = '<?= base_url('share/ujian/play/' . $ujian['token']) ?>';
                        }, 2000);
                    }
                },
                error: function(xhr, status, error) {
                                ...
                                if (res.tampil_nilai == 1) {
                                    htmlContent = `
                                        <div style="text-align: left; font-size: 16px; line-height: 1.6;">
                                            ...
                                            <p><strong>Jumlah Soal:</strong> ${res.jumlah_soal}</p>
