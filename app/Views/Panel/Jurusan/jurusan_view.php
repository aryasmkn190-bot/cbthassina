    ...
    <div class="middle-content container-xxl p-0">
        <div class="layout-top-spacing">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
                <h5 class="mb-2 mb-sm-0">Manajemen Jurusan</h5>
            ...
            <div class="input-group mb-3">
                ...
                <input type="text" id="searchBox" class="form-control" placeholder="Cari jurusan...">
            </div>

            <!-- List -->
            <div id="jurusanList" class="row g-3"></div>
...
<script>
    ...
    // Render single card
    function renderJurusanCard(jurusan) {
        return `
    <div class="col-12 col-sm-6 col-md-3">
        <div class="card h-100 shadow-sm position-relative p-2">

            <div class="card-body d-flex flex-column">
                <h6 class="card-title mb-2 text-truncate">${jurusan.nama}</h6>
                <span class="badge ${jurusan.is_active == 1 ? 'bg-success' : 'bg-danger'} rounded-pill px-2 py-1 small">
                    ${jurusan.is_active == 1 ? 'Aktif' : 'Nonaktif'}
                </span>
            </div>

             <!-- Tombol edit/hapus di pojok kanan atas -->
            <div class="position-absolute top-0 end-0 d-flex gap-1 m-2">
                <button class="btn btn-sm edit-btn p-1" data-id="${jurusan.id}" title="Edit">
                    Edit
                </button>
                <button class="btn btn-sm delete-btn p-1" data-id="${jurusan.id}" title="Hapus">
                    Hapus
                </button>
            </div>
        </div>
    ...
    function loadJurusan() {
        $.get('<?= base_url('panel/jurusan/list') ?>', function(res) {
            if (res.status) {
                jurusanData = res.data;
                renderJurusanList();
            }
        });
    }
    ...
    function renderJurusanList() {
        ...
        const filtered = jurusanData.filter(j => j.nama.toLowerCase().includes(query));
        ...
        $('#jurusanList').html(filtered.slice(start, end).map(renderJurusanCard).join(''));
    ...
    function save() {
        const url = save_method === 'add' ?
            '<?= base_url('panel/jurusan/create') ?>' :
            '<?= base_url('panel/jurusan/update') ?>/' + $('[name="id"]').val();
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post(`<?= base_url('panel/jurusan/delete/') ?>${id}`, function(res) {
                    jurusanData = jurusanData.filter(j => j.id != id);
                    renderJurusanList();
                    Snackbar.show({
                        text: res.message,
                        pos: 'top-center'
