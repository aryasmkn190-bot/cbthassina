        ...
        <div class="layout-top-spacing">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    ...
                    <a href="<?= base_url('panel/banksoal/soal/print/') . $bank_soal_id ?>" target="_blank" class="btn btn-secondary btn-sm">
...
<script>
    ...
    function renderOpsi(value = '', jenis = 'pg', bobot = 0, isTrue = false, opsiId = '') {
        ...
        if (jenis === 'pg') {
            kunciInput = `
            <input type="radio" id="${uniqueId}" name="kunci" class="btn-check kunci-input" ${isTrue ? 'checked' : ''}>
    ...
    <div class="row align-items-center mb-2 opsi-item" data-id="${opsiId}">
        ...
        <div class="col">
            <div class="quill-wrapper">
                <div class="quill-opsi-editor" data-value="${value}" tabindex="0"></div>
            </div>
            <input type="hidden" class="pilihan-input" value="${value}">
        </div>
        <div class="col-2">
    ...
    function initFormulaModal(quillInstance) {
        ...
        saveBtn.addEventListener('click', () => {
            ...
            if (currentFormulaEl) {
                ...
                quillInstance.insertEmbed(range.index, 'formula', latex, Quill.sources.USER);
        ...
        // fungsi global open
        window.openFormulaModal = function(quill, formulaEl = null) {
            ...
            const defaultLatex = formulaEl?.getAttribute('data-value');
            if (defaultLatex) {
                katex.render(defaultLatex, preview, {
                    throwOnError: false
                });
            }
    ...
    function initQuillPertanyaan() {
        ...
        const observer = new MutationObserver(() => {
            ...
            deleted.forEach(url => {
                if (url.startsWith(base + 'uploads/images/') || url.startsWith(base + 'uploads/audio/') || url.startsWith(base + 'uploads/video/')) {
                    ...
                    }).then(res => res.json()).then(json => console.log('[RESPON DELETE]', json)).catch(err => console.error(err));
                }
            });
        ...
        // aktifkan modal formula
        initFormulaModal(quillMain);

        // ⬅️ Tambahin ini: klik formula di editor untuk edit
    ...
    function initQuillOpsi() {
        $('.quill-opsi-editor').each(function(i, el) {
            ...
            q.on('text-change', () => {
                $(el).closest('.opsi-item').find('.pilihan-input').val(q.root.innerHTML);
            });
        });
    }
