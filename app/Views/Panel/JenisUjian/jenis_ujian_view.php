    ...
    <div class="middle-content container-xxl p-0">
        <div class="layout-top-spacing">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
                <h5 class="mb-2 mb-sm-0">Manajemen Jenis Ujian</h5>
            ...
            <div class="input-group mb-3">
                ...
                <input type="text" id="searchBox" class="form-control" placeholder="Cari jenis ujian...">
            </div>

            <!-- List -->
            <div id="jenisList" class="row g-3"></div>
    ...
    <div class="modal-dialog">
        <form id="form" class="modal-content needs-validation" novalidate>
            <div class="modal-header">
                <h5 class="modal-title">Form Jenis Ujian</h5>
...
<script>
    ...
    // Render single card
    function renderJenisCard(item) {
        return `
        <div class="col-12 col-sm-6 col-md-3">
            <div class="card h-100 shadow-sm position-relative p-2">

                <div class="card-body d-flex flex-column">
                    <h6 class="card-title mb-1 text-truncate">${item.nama}</h6>
                    <small class="text-muted mb-2">${item.kode ?? '-'}</small>

                    <span class="badge ${item.is_active == 1 ? 'bg-success' : 'bg-danger'} rounded-pill px-2 py-1 small">
                ...
                <div class="position-absolute top-0 end-0 d-flex gap-1 m-2">
                    <button class="btn btn-sm edit-btn p-1" data-id="${item.id}" title="Edit">Edit</button>
                    <button class="btn btn-sm delete-btn p-1" data-id="${item.id}" title="Hapus">Hapus</button>
                </div>
            </div>
        </div>`;
    }
    ...
    // Load data
    function loadJenis() {
        $.get('<?= base_url('panel/jenis-ujian/list') ?>', function(res) {
            if (res.status) {
                jenisData = res.data;
                renderJenisList();
            }
    ...
    function renderJenisList() {
        ...
        const filtered = jenisData.filter(a => a.nama.toLowerCase().includes(query));
        ...
        $('#jenisList').html(filtered.slice(start, end).map(renderJenisCard).join(''));
    ...
    function save() {
        const url = save_method === 'add' ?
            '<?= base_url('panel/jenis-ujian/create') ?>' :
            '<?= base_url('panel/jenis-ujian/update') ?>/' + $('[name="id"]').val();
    ...
    $(document).on('click', '.edit-btn', function() {
        ...
        const item = jenisData.find(a => a.id == id);
        if (item) {
            $('[name="id"]').val(item.id);
    ...
    $(document).on('click', '.delete-btn', function() {
        const id = $(this).data('id');
        Swal.fire({
            title: 'Yakin hapus jenis ujian ini?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.post(`<?= base_url('panel/jenis-ujian/delete/') ?>${id}`, function(res) {
                    jenisData = jenisData.filter(a => a.id != id);
                    renderJenisList();
                    Snackbar.show({
                        text: res.message,
                        pos: 'top-center'
