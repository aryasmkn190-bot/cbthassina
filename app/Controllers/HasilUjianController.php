...
class HasilUjianController extends BaseController
{
    ...
    public function exportSkoring($ujianId)
    {
        ...
        $hasilList = $this->hasilUjianModel->getSkoringByUjianId($ujianId, $filters);
        if (!$hasilList) {
        ...
        foreach ($hasilList as $hasil) {
            ...
            foreach ($soalByTopik as $topikId => $daftarSoal) {
                ...
                // --- BAGIAN INI YANG DIPERBAIKI (START) ---
                
                // Ambil nama topik dengan aman menggunakan Null Coalescing Operator (??)
                // Jika $topikId tidak ada di $topikMap, gunakan teks default.
                $namaTopik = $topikMap[$topikId]['nama'] ?? 'Topik (ID: ' . substr($topikId, 0, 8) . '...)';

                // Catatan
                if ($maks > 0) {
                    if ($skor >= 80) $catatanList[] = 'Sangat baik di ' . $namaTopik;
                    elseif ($skor >= 60) $catatanList[] = 'Cukup di ' . $namaTopik;
                    else $catatanList[] = 'Kurang di ' . $namaTopik;
                }
                
                // --- BAGIAN INI YANG DIPERBAIKI (END) ---
        ...
        if (!empty($filters['kelas'])) {
            $kelasSuffix = '_' . url_title($filters['kelas'], '_', true);
        }
        $fileName = 'Skoring_' . url_title($ujian['nama_ujian'], '_', true) . $kelasSuffix . '.xlsx';
